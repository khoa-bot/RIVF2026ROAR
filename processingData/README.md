...
1. **The extreme set is not covered by any script.** `trp_results_ROAR_extremeDataset_modified.csv`
   and `log_report_ROAR_extremeDataset_modified.txt` are committed here, and `loops_creator.py` does
   read the log, but no script builds a `trp_summary_extreme.csv`. Table III (Draft NN vs. ROAR,
   reduction %) was assembled from the `RawCost` / `FinalCost` columns of that CSV by hand. If you
   need it automated, add an `"extreme"` entry to `FILES` with only the ROAR key and compute
   `(RawCost - FinalCost) / RawCost`.

2. **`OOM` is inferred, not observed.** Any GILS-RVND instance header with no runs after it is
   labelled `OOM`. A crash, a missing binary, or an interrupted script would be labelled the same
   way. Cross-check `dmesg` / the console before trusting the label.

3. **Every input is append-mode.** All three solvers append to their logs. If you re-run a benchmark
   without clearing the old file, the averages silently mix runs from different configurations. Clear
   or archive first.

4. **Run order is not enforced.** Nothing checks that `trp_summary_*.csv` exists before
   `generate_percentage.py` runs; you just get a `FileNotFoundError`. Follow §4.

5. **`merge_trivialCombination.py` joins on `(Dataset, Nodes)`.** ROAR writes `dsj1000` with
   `Nodes = 1000` in both CSVs, so it matches — but if you ever edit the `datasets` array in one
   `main_*.cpp` and not the other, rows will drop out of the join with no warning (`how='left'`).

6. **`loops_creator.py` resets `current_dataset` after each match**, so it expects exactly one
   `Total LNS Loops Executed` line per `=== Processing Dataset ===` header. That is what ROAR emits;
   a malformed log will under-count rather than misattribute.

7. **Fixed filenames everywhere.** None of the scripts take command-line arguments. Change the
   constants at the top of each file if your naming differs.
